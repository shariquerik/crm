...
<template>
  <Tooltip :text="field?.reason ?? ''" :disabled="editable">
    <div class="group relative size-20 shrink-0">
      ...
      <template v-else-if="editable">
        ...
        >
          <span
            class="lucide-camera size-4 text-ink-gray-6"
            aria-hidden="true"
          />
        </button>
      </template>
      ...
      >
        <span
          class="lucide-external-link size-4 text-white"
          aria-hidden="true"
        />
      </a>
...
import { useRecordImage } from '@/composables/useRecordImage'
...
const { field, image, sourceHref, setImage } = useRecordImage(
  () => props.doctype,
  doc,
)

const editable = computed(() => Boolean(field.value?.editable))
