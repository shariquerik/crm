...
export function useFittedActions(
  ...
  // Each drop is measured against real layout, so the row's own children decide the fit.
  // nextTick is a microtask: the browser never paints an oversized row.
  ...
  watch([total, fitting], fit, { flush: 'post' })
