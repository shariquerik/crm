...
export function useSavedViews(options: {
  ...
  const { ctx, doctype, viewName, currentView, filters, sort, columns } =
    options
  ...
  const viewLabel = computed(() => {
    ...
    const row = doctypeViews.value.find(
      (v: any) => String(v.name) === String(viewName),
    )
      ...
      options: [
        {
          label: 'Default view',
          onClick: () =>
            router.push(`/${encodeURIComponent(route.params.doctype)}`),
        },
        ...doctypeViews.value.map((view: any) => ({
          label: view.label,
          onClick: () =>
            router.push(
              `/${encodeURIComponent(route.params.doctype)}/view/${view.name}`,
            ),
        })),
      ],
    },
