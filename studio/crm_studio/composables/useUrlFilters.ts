...
export function useUrlFilters(options: {
  ...
  const liveRoute = () =>
    ((router.currentRoute as any)?.value ?? router.currentRoute) as any
  ...
  function syncUrl() {
    ...
    const owned = new Set(
      getFilterableFields(metaFields.value, doctype).map(
        (f: any) => f.fieldname,
      ),
    )
    ...
    for (const [fieldname, pair] of Object.entries(
      toFiltersDict(completeFilters(filters.value)),
    )) {
    ...
    written = encoded
    const structural =
      Object.keys(query).sort().join() !== Object.keys(current).sort().join()
